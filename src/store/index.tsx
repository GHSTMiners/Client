...
import { AavegotchiContractObject, ConsumableSchema, ConsumableRecord, IndexedArray, IndexedCrypto, IndexedItem, IndexedPlayers, IndexedString, InventoryExplosives, Item, PlayerVitals, InventoryConsumables } from 'types'
...
type State = {
    ...
    worldConsumables: InventoryConsumables,  
    totalValue: number,
    depth: number,
}
...
type Actions = {
    ...
    setConsumables: (key:string,consumableSchema : ConsumableSchema) => void
...
export const useGlobalStore = create<State & Actions>((set) => ({
    ...
    setWorldConsumables: (consumables) => { 
        set( () => ({ worldConsumables : consumables }) ) 
    },
    ...
    setUserShortcut: ( id:number , item) => {
        set( (state) => {
            ...
            let duplicateItemKey =  Object.keys(newState).find( key => newState[key].id === item.id && newState[key].type === item.type );
    ...
    setConsumables: (key,consumableSchema) => {
        set( (state) => { 
            // auto-assigning a console shortcut if a new explosive is purchased and space is available
            const userShortcuts = {...state.userShortcuts};
            const availableShortcut = Object.keys(userShortcuts).find( shortcutKey => userShortcuts[shortcutKey].id === undefined )
            const savedShortcut = Object.keys(userShortcuts).find( shortcutKey => userShortcuts[shortcutKey].id === +key )
            if ( availableShortcut && savedShortcut === undefined ) {
                const item = state.worldConsumables[+key];
                item.quantity = consumableSchema.amount;
                state.setUserShortcut(+availableShortcut,item)
            }
            return( 
                {consumables: {...state.consumables, [key]: consumableSchema}}
            ) 
        })
    },
