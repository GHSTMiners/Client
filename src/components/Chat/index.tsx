...
const Chat : React.FC<Props> = ({ gameMode=true }) => {
  ...
  function renderDeadMessage (message: Protocol.NotifyPlayerDied):JSX.Element{
    ...
    switch (+deathReason){
      case Protocol.DeathReason.Exploded:
          return(
            <span key={Date.now()} style={{color:'white'}}>
  ...
  return (
    <div  id="chat" 
      ...
      <button className={styles.closeButton} onClick={ ()=>Client.getInstance().phaserGame.events.emit( gameEvents.chat.HIDE ) } hidden={!chatVisibility.state}>X</button>      
