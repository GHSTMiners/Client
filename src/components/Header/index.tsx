...
export const Header = () => {
  ...
  const routeToLobby = () => {
    ...
    Client.getInstance().colyseusClient.joinOrCreate<Schema.Lobby>("Lobby").then(room => {
      console.log(`➡️ Joined lobby room`)
      setIsLoading(false)
      Client.getInstance().lobbyRoom = room 
      Client.getInstance().lobbyRoom.onLeave(code => {
        console.log(`⬅️ Left the lobby room with code: ${code}`)
      })
      navigator('/lobby')
    }).catch(exception => {
      setIsLoading(false)
      ...
      >
        ...
        <div className={styles.playButtonContainer} hidden = { inLobby || inEndgame}> 
          ...
          {serversOnline? 
            ...
            : <img src={temporarilyClosed} style={{height: '7.3rem', marginTop: '0.6rem' }} alt={'Temporarily closed for regular maintenance'}/> }
          </div>
          <WalletButton />
        </div>
