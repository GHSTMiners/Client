...
export default class LoadingScene extends Phaser.Scene {
    ...
    preload() {
        ...
        var progressBar = new Phaser.Geom.Rectangle(this.cameras.main.width/2-200, this.cameras.main.height/2-25, 400, 50);
        var progressBarFill = new Phaser.Geom.Rectangle(this.cameras.main.width/2-195, this.cameras.main.height/2-20, 290, 40);
        ...
        world.crypto.forEach(crypto => {
            ...
            this.load.image(`crypto_wallet_${crypto.id}`, `https://chisel.gotchiminer.rocks/storage/${crypto.soil_image}`);
        })
        world.soil.forEach(soil => {
            this.load.image(`soil_top_${soil.id}`, `https://chisel.gotchiminer.rocks/storage/${soil.top_image}`);
            this.load.image(`soil_middle_${soil.id}`, `https://chisel.gotchiminer.rocks/storage/${soil.middle_image}`);
            this.load.image(`soil_bottom_${soil.id}`, `https://chisel.gotchiminer.rocks/storage/${soil.bottom_image}`);
        })
