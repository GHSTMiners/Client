...
export const LeaderboardTable = ({pageIndex,entriesPerPage,highscores,category,ownedGotchis,onlyMine,competition}:Props) => {
  ...
  const renderRankingRow = ( rank:number, name:string, score: number, reward?:ReactNode, wallet?:string, tokenId?:string) =>{
    //console.log(`#${rank} - [${tokenId}] ${name}: ${wallet}`)
    return(
  ...
  let leaderboardDisplayData = displayedScores?.map( (row,i) => {
    return( renderRankingRow( row.position, row.name, row.score, row.reward , row.wallet, row.tokenId) )
  });
