...
export const getPreviewGotchi = async (
  ...
  const svg = await callDiamond<Tuple<string, 4>>(provider, {
...
export const getHighScoresWithNames = async ( gotchiIDs: string[], displayData: HighScore[]): Promise<Array<HighScore>> => {
  ...
  try {
    // Fetch data for each chunk of 1000 elements
    for (let i = 0; i < gotchiIDs.length; i += 1000) {
      const chunk = gotchiIDs.slice(i, i + 1000);
      const res = await callSubgraph<AavegotchisNameArray>(
        getAavegotchiNames(chunk)
      );
      if (res) {
        res.aavegotchis.forEach((entry) => {
          const unnamedEntry = updateList.find(
            (oldEntry) => oldEntry.tokenId === entry.id
          );
...
export const formatHighScoreData = async ( gotchiIDs:string[], values:number[] ): Promise<Array<HighScore>> => {
  ...
  try {
    ...
    console.log('>> The Aavegotchi Subgraph API is not responding')
    console.log(err);
  }
  return updateList
};
